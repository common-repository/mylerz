...
async function GetCityZoneList() {
  // console.log(config.shipping_neighborhood);
  ...
  UpdateBillingNeighborhood(cityNeighborhoodObject,config.billing_neighborhood);
  UpdateBillingCompound(config.billing_compound)
  UpdateShippingNeighborhood(cityNeighborhoodObject,config.shipping_neighborhood);
  UpdateShippingCompound(config.shipping_compound)

  jQuery("#_billing_state").change(function () {
    UpdateBillingNeighborhood(cityNeighborhoodObject);
    UpdateBillingCompound()

  });

  jQuery("#_shipping_state").change(function () {
    UpdateShippingNeighborhood(cityNeighborhoodObject);
    UpdateShippingCompound()

  });

  jQuery('#_billing_neighborhood').change(function() {
    UpdateBillingCompound()
 });
  jQuery('#_shipping_neighborhood').change(function() {
    UpdateShippingCompound()
 });
...
function UpdateBillingNeighborhood(cityNeighborhoodObject, selectedNeighborhood) {
  let city = jQuery("#_billing_state option:selected").text();
  ...
  jQuery("#_billing_neighborhood").val(selectedNeighborhood).change();
}
function UpdateShippingNeighborhood(cityNeighborhoodObject, selectedNeighborhood) {
  let city = jQuery("#_shipping_state option:selected").text();
  let mylerzCity = WoocommerceToMylerzCities[city];
  let selectedZones = cityNeighborhoodObject[mylerzCity];
  SetShippingNeighborhoodOptions(selectedZones);
  jQuery("#_shipping_neighborhood").val(selectedNeighborhood).change();

}


function UpdateBillingCompound(SelectedCompound) {
  let neighborhood = jQuery("#_billing_neighborhood option:selected").text();
    if (NeighborhoodToCompoundListMapping[neighborhood]==undefined) {
        SetBillingCompoundOptions({"":""})
       jQuery('#_billing_compound_field').hide();
      } else {
        SetBillingCompoundOptions(NeighborhoodToCompoundListMapping[neighborhood])
        jQuery('#_billing_compound_field').show();
        jQuery("#_billing_compound").val(SelectedCompound).change();
    }
}
function UpdateShippingCompound(SelectedCompound) {
  let neighborhood = jQuery("#_shipping_neighborhood option:selected").text();
    if (NeighborhoodToCompoundListMapping[neighborhood]==undefined) {
        SetShippingCompoundOptions({"":""})
       jQuery('#_shipping_compound_field').hide();
      } else {
        SetShippingCompoundOptions(NeighborhoodToCompoundListMapping[neighborhood])
        jQuery('#_shipping_compound_field').show();
        jQuery("#_shipping_compound").val(SelectedCompound).change();
    }
}


function SetBillingNeighborhoodOptions(zones) {
  let el = jQuery("#_billing_neighborhood");
...
function SetShippingNeighborhoodOptions(zones) {
  let el = jQuery("#_shipping_neighborhood");
  el.empty(); // remove old options
  el.append(jQuery("<option></option>"))
  jQuery.each(zones, function (key, value) {
    el.append(jQuery("<option></option>").attr("value", value).text(key));
...
function SetBillingCompoundOptions(zones) {
  let el = jQuery("#_billing_compound");
  el.empty(); // remove old options
  el.append(jQuery("<option></option>"))
  jQuery.each(zones, function (key, value) {
    el.append(jQuery("<option></option>").attr("value", value).text(key));
...
function SetShippingCompoundOptions(zones) {
  let el = jQuery("#_shipping_compound");
  el.empty(); // remove old options
  el.append(jQuery("<option></option>"))
  jQuery.each(zones, function (key, value) {
    el.append(jQuery("<option></option>").attr("value", value).text(key));
